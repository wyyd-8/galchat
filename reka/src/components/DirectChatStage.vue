...
<template>
  <main class="chat-page direct-chat-page">
    <header class="chat-header">
      ...
      <div class="chat-header-actions"><span class="live-status active"><i />{{ world.thinkStatus === false ? (world.eotDetectionStatus ? '自动识别输入结束' : '连续消息模式') : '逐步显示思考与回复' }}</span><button class="button ghost" @click="emit('edit')"><Settings2 :size="16" />角色设置</button></div>
    </header>
