    ...
    <TabsRoot default-value="status" class="tabs trpg-tools">
      ...
      <TabsContent value="status" class="tabs-content tool-section">
        <section class="tool-card">
          <div class="tool-card-heading"><span><strong>KP 上下文占用</strong><small>{{ contextUsage ? `${contextUsage.characterCount.toLocaleString()} / ${contextUsage.softLimit.toLocaleString()} 字符` : '尚无记录' }}</small></span><button class="icon-button bordered" :disabled="busy" title="刷新" @click="execute(refreshOverview)"><RefreshCw :size="15" /></button></div>
        ...
        <section class="tool-card">
          ...
          <div class="tool-actions"><button class="button secondary" :disabled="busy" @click="execute(saveSnapshot)"><Save :size="16" />覆盖存档</button><button class="button" :class="confirmLoad ? 'danger' : 'ghost'" :disabled="!save || busy" @click="execute(loadSnapshot)"><RotateCcw :size="16" />{{ confirmLoad ? '再次点击确认读档' : '读取存档' }}</button></div>
      ...
      <TabsContent value="card" class="tabs-content tool-section">
        <label class="field"><span>查看对象</span><select v-model="selectedParticipant"><option value="">玩家调查员</option><option v-for="item in characters" :key="item.characterId" :value="String(item.characterId)">{{ item.characterName }}（BOT）</option></select></label>
        <section v-if="card" class="character-sheet">
          <div class="sheet-heading"><span><small>{{ card.character.actorType }} · {{ card.character.occupation || '未填写职业' }}</small><h3>{{ card.character.name }}</h3><p>{{ card.character.sex || '—' }} · {{ card.character.age || '—' }} 岁 · {{ card.character.era || '时代未填' }}</p></span><div class="vitals"><b>HP {{ card.character.hpCurrent }}/{{ card.character.hpMax }}</b><b>SAN {{ card.character.sanCurrent }}/{{ card.character.sanMax }}</b><b>MP {{ card.character.mpCurrent }}/{{ card.character.mpMax }}</b></div></div>
        ...
        <section v-else class="tool-card import-card"><strong>{{ selectedActorName }}尚未建立人物卡</strong><p>粘贴固定格式文本，至少包含姓名、职业、性别、年龄和八项基础属性。</p><label class="field"><span>人物卡文本</span><textarea v-model="cardText" rows="10" placeholder="调查员, 记者, 女, 27岁\n时代: 1920s\nSTR 50 CON 55 SIZ 60 DEX 65 APP 60 INT 70 POW 55 EDU 70\n——技能——\n侦查 60%" /></label><button class="button primary" :disabled="!cardText.trim() || busy" @click="execute(createCard)">导入人物卡</button></section>
      </TabsContent>

      <TabsContent value="dice" class="tabs-content tool-section">
        <div class="field-inline"><label class="field"><span>掷骰摘要 ID</span><input v-model="diceId" inputmode="numeric" placeholder="由跑团消息中的骰子事件提供" /></label><button class="button secondary dice-load-button" :disabled="!diceId || busy" @click="execute(() => loadDice())"><RefreshCw :size="16" />查询</button></div>
        <section v-if="diceSummary" class="tool-card dice-summary"><div class="tool-card-heading"><span><strong>{{ diceSummary.reason || `掷骰 #${diceSummary.id}` }}</strong><small>{{ diceSummary.status }} · {{ diceSummary.roundCount || 1 }} 轮</small></span><b>{{ diceSummary.totalResult || '进行中' }}</b></div></section>
        ...
        <div v-else class="empty-panel compact"><Dices :size="24" /><p>出现掷骰事件后会自动显示，也可输入摘要 ID 查询。</p></div>
      </TabsContent>
    </TabsRoot>
