...
public class GroupDiceMessageFormatter {
    ...
    public String format(String messageContent) {
        ...
        for (Integer roundNo : reference.roundNos()) {
            List<DiceRollResult> round = results.stream()
                    .filter(result -> Objects.equals(roundNo, result.getRoundNo()))
                    .toList();
            String reasons = round.stream()
                    .map(DiceRollResult::getReason)
                    .filter(StringUtils::hasText)
                    .distinct()
                    .collect(Collectors.joining("；"));
            content.append("\n第").append(roundNo).append("轮：\n- ");
            if (StringUtils.hasText(reasons)) {
                content.append(reasons).append("：");
            }
            if (round.stream().anyMatch(result -> result.getResolvedAt() == null)) {
                content.append("等待玩家掷骰");
            } else {
                content.append(summaryFormatter.formatRound(round));
            }
        }
