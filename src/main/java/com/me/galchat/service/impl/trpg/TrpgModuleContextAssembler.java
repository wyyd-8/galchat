...
public class TrpgModuleContextAssembler {
    ...
    private void appendProgressBoundaryRules(StringBuilder result) {
        result.append("""
                ...
                “最终场景说明”同属非剧情标注：按其中的完结条件、收尾范围和继续条件判断正篇是否结束；其中规定的收尾行动不适用强制创建子场景规则。
                满足“最终场景说明”的完结条件时，在可调用finishRun的探索或战后过渡步骤简短收尾并调用finishRun，不以finishSceneExploration代替整个跑团的完结；人物后传交由系统处理。
