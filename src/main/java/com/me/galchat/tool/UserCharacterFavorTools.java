...
public class UserCharacterFavorTools {
    ...
    @Tool(description = """
            工具描述：
            调整当前角色对用户的好感值。
            当用户本轮行为、表达、选择或承诺会让当前角色产生明确好感变化时，必须调用本工具。
            使用流程：
            1. 先判断当前用户消息是否让角色更信任、感动、安心、失望、受伤、生气或疏远；如果是，先调用本工具，再回复。
            2. favorChange 传入本次期望变化值，正数表示增加好感，负数表示降低好感。
            3. 该方法没有返回值。
            注意事项：
            仅在当前对话内容确实触发好感变化时调用，不要因为普通寒暄、信息查询或无明显态度变化的内容调用。
            数值应克制：轻微触动使用小幅变化，明显善意/伤害使用中等变化，重大信任或背叛才使用较大变化。
            """)
