...
CREATE TABLE group_chat_reply_step (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_group_chat_reply_step_execution_mode CHECK (
...
CREATE INDEX idx_binding_chat
    ON user_character_favor_log (binding_chat);
...
CREATE TABLE trpg_investigator_suspension (
    id BIGSERIAL PRIMARY KEY,
    conversation_id BIGINT NOT NULL,
    ...
    recovery_scene_name VARCHAR(200),
...
CREATE INDEX idx_trpg_investigator_suspension_state
